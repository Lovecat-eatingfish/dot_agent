...
from .hooks import Hook, HookEvent, HookPayload, HookResult, HookRunner
from .log import get_logger
from .utils import json_safe

logger = get_logger(__name__)

_GLOBAL_HOOKS = Path.home() / ".dot" / "hooks.json"
_DEFAULT_TIMEOUT = 30
...
def _project_hooks_path(workspace: Path | None) -> Path:
    if workspace is None:
        return Path.cwd() / ".dot" / "hooks.json"
    return workspace / ".dot" / "hooks.json"
...
def _make_prompt_handler(prompt_template: str):
    ...
    - LLM 不可用 / 返回非 JSON → 不阻断（fail-open）
    """
    def handler(payload: HookPayload) -> HookResult:
