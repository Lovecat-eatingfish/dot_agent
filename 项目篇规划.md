...
```
在 src/mokioclaw/orchestration/agent.py 中实现一个 ReAct 循环：
...
**图构建 — [workflow.py:24](src/mokioclaw/orchestration/workflow.py#L24)**
...
**状态定义 — [state.py:60](src/mokioclaw/state/graph.py#L60)**
...
**Verifier 路由 — [nodes.py:412](src/mokioclaw/orchestration/nodes.py#L412)**
...
**Verifier 节点 — [nodes.py:221](src/mokioclaw/orchestration/nodes.py#L221)**
...
```
在 src/mokioclaw/state/graph.py 中定义 LangGraph 图的共享状态：
...
```
在 src/mokioclaw/orchestration/nodes.py 中实现三个核心节点：
...
```
在 src/mokioclaw/orchestration/workflow.py 中构建 LangGraph 图：
...
**Planner 的 Agent 调度工具 — [nodes.py:473](src/mokioclaw/orchestration/nodes.py#L473)**
...
**SearchAgent Handoff — [nodes.py:536](src/mokioclaw/orchestration/nodes.py#L536)**
...
**CodeAgent Handoff — [nodes.py:558](src/mokioclaw/orchestration/nodes.py#L558)**
...
```
改造 src/mokioclaw/orchestration/nodes.py 中的 planner_node：
...
**三层 Memory 构建 — [memory.py:39](src/mokioclaw/memory/memory.py#L39)**
...
**Context Monitor — [nodes.py:327](src/mokioclaw/orchestration/nodes.py#L327)**
...
**Token 估算 — [nodes.py:460](src/mokioclaw/orchestration/nodes.py#L460)**
...
**Context Compressor — [nodes.py:356](src/mokioclaw/orchestration/nodes.py#L356)**
...
**LLM 压缩核心 — [nodes.py:616](src/mokioclaw/orchestration/nodes.py#L616)**
...
```
在 src/mokioclaw/memory/memory.py 中实现三层 Memory 系统：
...
```
在 src/mokioclaw/orchestration/nodes.py 中新增 context_monitor_node：
...
```
在 src/mokioclaw/orchestration/nodes.py 中新增 context_compressor_node：
...
```
在 src/mokioclaw/orchestration/workflow.py 中更新图结构：
...
**命令风险分类 — [approval.py:44](src/mokioclaw/security/approval.py#L44)**
...
**Checkpoint 保存 — [checkpoint.py:47](src/mokioclaw/reliability/checkpoint.py#L47)**
...
**主循环中的 Checkpoint + Trace 集成 — [agent.py:291](src/mokioclaw/orchestration/agent.py#L291)**
...
```
在 src/mokioclaw/security/approval.py 中实现命令风险分类和审批：
...
```
在 src/mokioclaw/reliability/checkpoint.py 中实现断点保存和恢复：
...
```
在 src/mokioclaw/reliability/trace.py 中实现执行追踪：
...
```
更新 src/mokioclaw/orchestration/agent.py 中的 stream_agent_events：
...
```
在 src/mokioclaw/interaction/app.py 中新增参数：
...
**Intent Router — [nodes.py:85](src/mokioclaw/orchestration/nodes.py#L85)**
...
**入口工作流 — [workflow.py:49](src/mokioclaw/orchestration/workflow.py#L49)**
...
**多轮对话 Session — [agent.py:153](src/mokioclaw/orchestration/agent.py#L153)**
...
**TUI 审批弹窗 — [tui/approval.py:34](src/mokioclaw/interaction/tui/approval.py#L34)**
...
```
在 src/mokioclaw/orchestration/workflow.py 中新增入口工作流图：
...
在 src/mokioclaw/orchestration/nodes.py 中实现两个新节点：
...
```
在 src/mokioclaw/reliability/session.py 中实现会话管理：
...
```
在 src/mokioclaw/orchestration/agent.py 中新增 stream_session_events：
...
```
在 src/mokioclaw/interaction/tui/app.py 中用 textual 实现 TUI：
...
审批弹窗实现（src/mokioclaw/interaction/tui/approval.py）：
...
```
在 src/mokioclaw/interaction/tui/logo.py 中实现 ASCII Art Logo：
...
| [workflow.py:24](src/mokioclaw/orchestration/workflow.py#L24) | `build_complex_workflow` | 2-6 | 图结构定义：节点 + 边 + 条件路由 |
| [workflow.py:49](src/mokioclaw/orchestration/workflow.py#L49) | `build_entry_workflow` | 6 | 入口图：intent_router → chat/workflow |
| [state.py:60](src/mokioclaw/state/graph.py#L60) | `MokioGraphState` | 2-6 | 图共享状态，`messages` 用 `add_messages` |
| [nodes.py:85](src/mokioclaw/orchestration/nodes.py#L85) | `intent_router_node` | 6 | LLM 判断意图：chat 还是 workflow |
| [nodes.py:152](src/mokioclaw/orchestration/nodes.py#L152) | `planner_node` | 2-6 | Supervisor：调用 TodoWrite/Search/Code 工具 |
| [nodes.py:221](src/mokioclaw/orchestration/nodes.py#L221) | `verifier_node` | 2-6 | 只读工具验证 + JSON 判断 pass/fail |
| [nodes.py:412](src/mokioclaw/orchestration/nodes.py#L412) | `verifier_route` | 2-6 | passed→final / 达上限→final / 否则→planner |
| [nodes.py:473](src/mokioclaw/orchestration/nodes.py#L473) | `_build_planner_tools` | 3 | Planner 的 3 个调度工具 |
| [nodes.py:536](src/mokioclaw/orchestration/nodes.py#L536) | `_call_search_agent_tool` | 3 | Handoff: planner→searchAgent |
| [nodes.py:558](src/mokioclaw/orchestration/nodes.py#L558) | `_call_code_agent_tool` | 3 | Handoff: planner→codeAgent |
...
| [memory.py:39](src/mokioclaw/memory/memory.py#L39) | `build_layered_memory` | 4 | 三层 Memory：Rules + Working + History |
| [nodes.py:327](src/mokioclaw/orchestration/nodes.py#L327) | `context_monitor_node` | 4 | 估算 token 数，决定是否压缩 |
| [nodes.py:356](src/mokioclaw/orchestration/nodes.py#L356) | `context_compressor_node` | 4 | LLM 摘要 + RemoveMessage 清空历史 |
| [nodes.py:616](src/mokioclaw/orchestration/nodes.py#L616) | `_compress_context_with_model` | 4 | 压缩核心：给 LLM 全量消息，返回摘要 |
| [notepad_tool.py:21](src/mokioclaw/tools/notepad_tool.py#L21) | `append_notepad` | 4 | 追加写入 NOTEPAD.md，压缩后不丢失 |
| [approval.py:44](src/mokioclaw/security/approval.py#L44) | `classify_command_risk` | 5 | 正则匹配高风险命令 |
...
| [checkpoint.py:47](src/mokioclaw/reliability/checkpoint.py#L47) | `CheckpointManager.save` | 5 | light/strict/off 三级检查点 |
| [trace.py:29](src/mokioclaw/reliability/trace.py#L29) | `TraceRecorder` | 5 | 逐事件记录 + 统计 + timeline.md |
| [agent.py:153](src/mokioclaw/orchestration/agent.py#L153) | `stream_session_events` | 6 | 多轮对话：session→router→workflow→session |
| [session.py:100](src/mokioclaw/reliability/session.py#L100) | `build_session_context` | 6 | 从 session.json 构建上下文字符串 |
| [tui/approval.py:34](src/mokioclaw/interaction/tui/approval.py#L34) | `ApprovalGate` | 6 | 线程同步：等待人类审批决策 |
