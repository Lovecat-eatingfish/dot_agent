...
def auto_memory_summary(workspace: Path) -> str:
    ...
    return "\n".join(lines)
# ---------------------------------------------------------------------------
# 后台记忆提取 + AutoDream（对齐 Claude Code）
# 1. 用户显式「记住」→ 主 Agent 直接写 topic
# 2. 每轮结束后后台提取（禁止 bash/MCP/再派子 Agent，最多 8 条）
# 3. AutoDream：距上次 ≥24h 且累计 ≥5 次会话时整理记忆
# （e77f38b 重构时误删，调用方 agent.py 与 tests/test_auto_memory_llm.py 仍依赖，恢复）
# ---------------------------------------------------------------------------
import threading
from datetime import datetime, timezone

from mokioclaw.core.utils import parse_json_content, truncate, write_json
...
def _run_autodream(workspace: Path) -> None:
    ...
    with store._write_lock:
        ...
        # 截断超长主题正文（保留 frontmatter + 前 8KB）——topic 文件与
        # write_topic / 后台提取写同一批文件，必须同锁，否则会互相覆盖半截内容
        for topic in topics:
            try:
