...
def write_todos(
    ...
    :param todos: 计划列表
    :param acceptance_criteria: 验收的标准列表
    :param verification_commands: 验证命令列表
    :return:
    """
...
def _normalize_items(items: Any) -> list[str]:
    ...
    if isinstance(items, dict):
        value = items.get("content") or items.get("description") or items.get("title") or items.get(
            "text") or items.get("command")
        if value:
