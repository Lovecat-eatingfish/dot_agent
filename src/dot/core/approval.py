"""
命令风险分类与审批数据结构（dot 独立副本）

- classify_command_risk: 高风险命令正则识别（pip install / curl / git push 等）
- ApprovalRequest / ApprovalDecision: 审批流程数据结构
- resolve_approval: 按审批模式解析结果
"""
...
# 前缀分隔符：命令起始 / 换行 / && / || / ; / 单管道 |
# （漏掉 | 和换行会让 "echo x | curl ..." 与多行命令第二行的 pip install 免审批）
RISK_PATTERNS = [
