...
3. 并发保护：通过 FileSnapshot 机制防止覆盖他人修改（先读后写）
4. 差异追踪：写入/编辑操作返回 unified diff

工具：read_file / write_file / edit_file（由 registry 包装成 StructuredTool）
"""
...
def _strip_workspace_prefix(file_path: str) -> str:
    """移除路径中的 workspace/ 前缀（LLM 有时会加这种冗余前缀）"""
