...
from core.state import DotAgentGraphState, SourceItem

from typing import Callable, Any
import json
from model.openai_provider import create_model
from prompt.prompt import SEARCH_AGENT_PROMPT
...
def _dedupe_sources(sources: list[dict[str, Any]]) -> list[SourceItem]:
    seen: set[str] = set()
    deduped = []
    for source in sources:
        ...
        if not url or url in seen:
            continue
        seen.add(url)
        item = SourceItem(source.get("title"), source.get("url"), source.get("content"), source.get("score"))
        deduped.append(item)
    return deduped
