...
def bash_tool_description() -> str:
    ...
    common = (
        "在工作区内运行一个安全的开发 Shell 命令，支持超时设置和输出捕获。"
        "该命令已默认将工作目录设为工作区，因此请使用相对路径，不要执行 cd /workspace、"
        "cd workspace 或长时间运行的交互式命令。每次调用都会启动一个新的 Shell；导出的环境变量不会在"
        "调用之间持久化，因此请将可复用的环境值写入已配置的环境文件，或通过内联方式传递。"
        "长时间运行的服务应使用 run_in_background=true。对于文件检查，优先使用跨平台的 Python 单行命令。"
    )
    if system == "windows":
        return (
                common
                + " 当前平台：Windows。命令通过 cmd.exe 执行，而非 bash 或 PowerShell。"
                  "请使用 Windows cmd 语法：dir 列出目录，type file.txt 打印文件内容，copy/move/del 执行简单文件操作，"
                  "&& 用于命令链，set VAR=value 用于设置环境变量。除非你用 python -c 实现相应行为，否则不要使用仅适用于 POSIX 的工具，"
                  "如 tail、grep、sed、awk、cat、ls、export 或 here-document。"
        )
    if system == "darwin":
        return (
                common
                + " 当前平台：macOS。命令通过 POSIX Shell 执行。"
                  "请使用可移植的 sh/bash 风格命令，如 ls、cat、grep、tail、export，以及可用的 python/python3。"
        )
    return (
            common
            + " 当前平台：Linux/Unix。命令通过 POSIX Shell 执行。"
              "请使用可移植的 sh/bash 风格命令，如 ls、cat、grep、tail、export，以及可用的 python/python3。"
    )
...
# 命令转换操作：这个函数的核心目的是让同一个命令在 Windows 和 Unix-like 系统上都能工作：
# 主要是适配不同的命令
# 原始命令（Linux风格）	Windows转换后
# python3 script.py	python script.py
# cd /workspace && python run.py	python run.py
# pwd	cd
# ls -la	dir
# cat file.txt	type file.txt
# 这样用户（或AI）可以输入 Linux 风格的命令，函数会自动适配到 Windows 环境，提高跨平台兼容性。
...
# 这个函数 _handle_tail_command 的作用是模拟 Linux 的 tail 命令功能，用于读取文件的最后 N 行内容。它不实际调用系统命令，而是通过 Python 代码直接实现，以提高跨平台兼容性（Windows 上原生没有 tail 命令）。
def _handle_tail_command(state: RuntimeState, command: str) -> dict[str, Any] | None:
    ...
    from tools.file_tool import read_text_lossy, resolve_workspace_path
...
# 这个函数 _handle_workspace_query 的作用是模拟 cd 和 pwd 命令，但并不是真正地切换目录，而是返回当前工作区路径。
