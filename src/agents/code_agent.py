...
from core.node import format_layered_memory_for_prompt
from core.state import DotAgentGraphState, RuntimeState
from memory.memory import build_layered_memory, memory_event
from model.openai_provider import create_model
from prompt.prompt import CODE_AGENT_PROMPT
from tools.registry import build_tools
from tools.todo_tool import persist_todos, update_todo
...
def run_code_agent(
    ...
    for _ in range(max_loops):
        ...
        for call in tool_calls:
            ...
            writer(event)
            # 如果agent 节点 有更新 todolist 的工具 就去持久化一下
...
def _code_agent_input(state: DotAgentGraphState, instruction: str, memory: dict[str, Any]) -> str:
    parts = [
        f"Task: {state['task']}",
